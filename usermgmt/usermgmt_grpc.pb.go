...
type UserManagementClient interface {
	...
	Upload(ctx context.Context, opts ...grpc.CallOption) (UserManagement_UploadClient, error)
}
...
func (c *userManagementClient) Upload(ctx context.Context, opts ...grpc.CallOption) (UserManagement_UploadClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserManagement_ServiceDesc.Streams[0], "/usermgmt.UserManagement/Upload", opts...)
	if err != nil {
		return nil, err
	}
	x := &userManagementUploadClient{stream}
	return x, nil
}

type UserManagement_UploadClient interface {
	Send(*Chunk) error
	CloseAndRecv() (*UploadStatus, error)
	grpc.ClientStream
}

type userManagementUploadClient struct {
	grpc.ClientStream
}

func (x *userManagementUploadClient) Send(m *Chunk) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userManagementUploadClient) CloseAndRecv() (*UploadStatus, error) {
...
type UserManagementServer interface {
	...
	Upload(UserManagement_UploadServer) error
...
func (UnimplementedUserManagementServer) Upload(UserManagement_UploadServer) error {
...
func _UserManagement_Upload_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserManagementServer).Upload(&userManagementUploadServer{stream})
}

type UserManagement_UploadServer interface {
	SendAndClose(*UploadStatus) error
	Recv() (*Chunk, error)
	grpc.ServerStream
}

type userManagementUploadServer struct {
	grpc.ServerStream
}

func (x *userManagementUploadServer) SendAndClose(m *UploadStatus) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userManagementUploadServer) Recv() (*Chunk, error) {
	m := new(Chunk)
...
var UserManagement_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Upload",
			Handler:       _UserManagement_Upload_Handler,
			ClientStreams: true,
		},
	},
